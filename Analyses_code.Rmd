...
```{r , eval = FALSE, echo = FALSE, fig.width=12, fig.height=15, warning = FALSE, message= FALSE}
p2 <- ggplot(long_data2_f5, aes(x = variable, y = log2(value))) +
  geom_violin() +
  stat_summary(geom = "point", fun = median) +
...
col.group <- Sum_Expe_filtered$group_name
levels(col.group) <-  brewer.pal(nlevels(col.group), "Set1")
col.group <- as.character(col.group)
#col.group <- as.character(Sum_Expe_filtered$group)
plotMDS(assays(Sum_Expe_filtered)$counts, labels = Sum_Expe_filtered$Sample_ID, col=col.group, top = 500, dim.plot = c(2,1))
```
